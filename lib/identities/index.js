...
class Identities {
  constructor(storage, didm, ipfs) {
    ...
    _getIdentityByDid.set(this, {
      writable: true,
      value: (did) =>
        _classPrivateFieldGet(this, _identitiesList).find(
          (identity) => identity.getDid() === did
        ),
    });
    ...
    _assertDidmSupport.set(this, {
      ...
      value: (didMethod, ...purposes) => {
        purposes.forEach((purpose) => {
          if (
            !_classPrivateFieldGet(this, _didm).isSupported(didMethod, purpose)
          ) {
            throw new _errors.UnsupportedDidMethodPurposeError(
              didMethod,
              purpose
            );
          }
    ...
    _buildIdentitiesList.set(this, {
      ...
      value: () => {
        _classPrivateFieldSet(
          this,
          _identitiesList,
          Object.values(_classPrivateFieldGet(this, _identitiesMap))
        );

        _classPrivateFieldGet(this, _identitiesList).sort(
          (identity1, identity2) =>
            identity1.getAddedAt() - identity2.getAddedAt()
        );
      },
    });
    ...
    _updateIdentitiesList.set(this, {
      ...
      value: (operation) => {
        ...
        _classPrivateFieldGet(this, _onChange).dispatch(
          _classPrivateFieldGet(this, _identitiesList),
          operation
        );
      },
    });
  ...
  async load() {
    ...
    if (!_classPrivateFieldGet(this, _identitiesLoad)) {
      _classPrivateFieldSet(
        this,
        _identitiesLoad,
        identityFns.loadIdentities(
          _classPrivateFieldGet(this, _storage),
          _classPrivateFieldGet(this, _didm),
          _classPrivateFieldGet(this, _ipfs)
        )
      );
    ...
    try {
      _classPrivateFieldSet(
        this,
        _identitiesMap,
        await _classPrivateFieldGet(this, _identitiesLoad)
      );
    } catch (err) {
    ...
    if (cleanLoad) {
      _classPrivateFieldGet(this, _onLoad).dispatch(
        _classPrivateFieldGet(this, _identitiesList)
      );
    }
  ...
  async peek(didMethod, params) {
    _classPrivateFieldGet(this, _assertDidmSupport).call(
      this,
      didMethod,
      "getDid",
      "resolve"
    ...
    await this.load();
    const did = await _classPrivateFieldGet(this, _didm).getDid(
      didMethod,
      params
    );
    const didDocument = await _classPrivateFieldGet(this, _didm).resolve(did);

    const identity = _classPrivateFieldGet(this, _getIdentityByDid).call(
      this,
      did
    );

    const profileDetails = identity
      ? identity.profile.getDetails()
      : await identityFns.peekProfileDetails(
          did,
          _classPrivateFieldGet(this, _ipfs)
        );
    return {
  ...
  async create(didMethod, params) {
    ...
    const { did, backupData } = await _classPrivateFieldGet(this, _didm).create(
      didMethod,
      params,
      (document) => {
  ...
  async remove(id, params) {
    ...
    if (!identity.isRevoked()) {
      ...
      await _classPrivateFieldGet(this, _didm).update(
        did,
        params,
        (document) => {
          document.revokePublicKey(
            identity.devices.getCurrent().didPublicKeyId
          );
        }
      );
    }

    await identityFns.removeIdentity(
      id,
      _classPrivateFieldGet(this, _storage),
      _classPrivateFieldGet(this, _ipfs)
    );
...
const createIdentities = (storage, didm, ipfs) =>
  new Identities(storage, didm, ipfs);
