...
const checkPassphraseStrength = passphrase => {
  const {
    ...
    feedback
  } = (0, _zxcvbn.default)(passphrase);
  const score = (0, _normalizeValue.default)(guessesLog10, [{
    value: 0,
    norm: 0
  }, {
    value: 4,
  ...
  const warning = feedback.warning ? parseMessage(feedback.warning) : null; // If the score is less than 50% and there's no warning nor suggestions, make sure we have one
...
var _default = checkPassphraseStrength;
